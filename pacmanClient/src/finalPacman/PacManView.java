...
public class PacManView extends Group {
	...
	public void update(PacManModel model) {
		...
		// for each ImageView, set the image to correspond with the CellValue of that
		// cell
		for (int row = 0; row < this.rowCount; row++) {
			for (int column = 0; column < this.columnCount; column++) {
				...
				// make ghosts "blink" towards the end of ghostEatingMode (display regular ghost
				// images on alternating updates of the counter)
				if (PacManModel.isGhostEatingMode()
						&& (Controller.getGhostEatingModeCounter() == 6 || Controller.getGhostEatingModeCounter() == 4
				...
				try {
					if (row == model.getPacmanLocation2().getX() && column == model.getPacmanLocation2().getY()
						...
						this.cellViews[row][column].setImage(this.pacmanRightImage);
							...
							&& PacManModel.getLastDirection2() == PacManModel.Direction.LEFT) {
						this.cellViews[row][column].setImage(this.pacmanLeftImage);
					} else if (row == model.getPacmanLocation2().getX() && column == model.getPacmanLocation2().getY()
							&& PacManModel.getLastDirection2() == PacManModel.Direction.UP) {
						this.cellViews[row][column].setImage(this.pacmanUpImage);
					} else if (row == model.getPacmanLocation2().getX() && column == model.getPacmanLocation2().getY()
							&& PacManModel.getLastDirection2() == PacManModel.Direction.DOWN) {
						this.cellViews[row][column].setImage(this.pacmanDownImage);
					}
				} catch (Exception e) {
				}
