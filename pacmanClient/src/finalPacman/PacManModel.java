...
public class PacManModel {
	...
	private static Point2D pacmanLocation2;
	private static Direction lastDirection2;
	...
	/**
	 * Configure the grid CellValues based on the txt file and place PacMan and
	 * ghosts at their starting locations. "W" indicates a wall, "E" indicates an
	 * empty square, "B" indicates a big dot, "S" indicates a small dot, "1" or "2"
	 * indicates the ghosts home, and "P" indicates Pacman's starting position.
	 *
	...
	/**
	 * Move PacMan based on the direction indicated by the user (based on keyboard
	...
	public void movePacman(Direction direction) {
		...
		// determine whether PacMan should change direction or continue in its most
		// recent direction
		// if most recent direction input is the same as previous direction input, check
		// for walls
		...
		// if most recent direction input is not the same as previous input, check for
		// walls and corners before going in a new direction
		else {
			// if PacMan would hit a wall with the new direction input, check to make sure
			// he would not hit a different wall if continuing in his previous direction
			if (grid[(int) potentialPacmanLocation.getX()][(int) potentialPacmanLocation.getY()] == CellValue.WALL) {
				...
				if (grid[(int) potentialPacmanLocation.getX()][(int) potentialPacmanLocation
						.getY()] == CellValue.WALL) {
					pacmanVelocity = changeVelocity(Direction.NONE);
					setLastDirection(Direction.NONE);
				} else {
...
//		이동이 끝났다면 결과를 서버에 보내 다른 클라이언트가 볼 수 있도록 합니다
		String str = pacmanLocation.toString().toString();

		int pointx = str.indexOf('x');
		int pointy = str.indexOf('y');

		int a = str.indexOf(',');
		Util.client.move(str.substring(pointx + 4, a) + "!!" + str.substring(pointy + 4, str.length() - 1) + "@@"
				+ currentDirection);
	}
	...
	/**
	 * Move a ghost to follow PacMan if he is in the same row or column, or move
	 * away from PacMan if in ghostEatingMode, otherwise move randomly when it hits
	 * a wall.
	 * 
	 * @param velocity the current velocity of the specified ghost
	 * @param location the current location of the specified ghost
	 * @return an array of Point2Ds containing a new velocity and location for the
	 *         ghost
	 */
	public Point2D[] moveAGhost(Point2D velocity, Point2D location) {
		Random generator = new Random();
		// if the ghost is in the same row or column as PacMan and not in
		// ghostEatingMode,
		// go in his direction until you get to a wall, then go a different direction
		// otherwise, go in a random direction, and if you hit a wall go in a different
		// random direction
		if (!ghostEatingMode) {
			...
			// move in a consistent random direction until it hits a wall, then choose a new
			// random direction
			else {
				Point2D potentialLocation = location.add(velocity);
				potentialLocation = setGoingOffscreenNewLocation(potentialLocation);
		...
		// if the ghost is in the same row or column as Pacman and in ghostEatingMode,
		// go in the opposite direction
		// until it hits a wall, then go a different direction
		// otherwise, go in a random direction, and if it hits a wall go in a different
		// random direction
		if (ghostEatingMode) {
			if (location.getY() == pacmanLocation.getY()) {
				if (location.getX() > pacmanLocation.getX()) {
	...
	/**
	 * Updates the model to reflect the movement of PacMan and the ghosts and the
	 * change in state of any objects eaten during the course of these movements.
	 * Switches game state to or from ghost-eating mode.
	 * 
	 * @param direction the most recently inputted direction for PacMan to move in
	 */
	public void step(Direction direction) {
		...
		// move ghosts and checks again if ghosts or PacMan are eaten (repeating these
		// checks helps account for even/odd numbers of squares between ghosts and
		// PacMan)
		this.moveGhosts();
		if (ghostEatingMode) {
			if (pacmanLocation.equals(ghost1Location)) {
	...
	/**
	 * Connects each direction to Point2D velocity vectors (Left = (-1,0), Right =
	 * (1,0), Up = (0,-1), Down = (0,1))
	 * 
	 * @param direction
	...
	public void setLocation(Point2D point) {
		pacmanLocation = point;
	}

	public static void setLocation2(Point2D point) {
		pacmanLocation2 = point;
	}
	...
	public static Direction getLastDirection2() {
	...
	public static void setLastDirection2(Direction direction) {
	...
	public Point2D getPacmanLocation2() {
		return pacmanLocation2;
	}
